...
const createOrderFromCart = asyncHandler(async (req, res) => {
  ...
  for (const cartItem of cart.items) {
    ...
    if (!product.isActive) {
      throw new ApiError(
        400,
        `Product "${product.productTitle}" is no longer available`
      );
    }
    ...
    if (product.quantity < cartItem.quantity) {
      throw new ApiError(
        400,
        `Insufficient stock for "${product.productTitle}". Available: ${product.quantity}, Requested: ${cartItem.quantity}`
      );
    }
    ...
    if (product.userId._id.toString() === userId.toString()) {
      throw new ApiError(
        400,
        `You cannot order your own product: "${product.productTitle}"`
      );
    }
    ...
    validatedItems.push({
      ...
      priceAtTime: product.price, // Use current price
      productSnapshot: {
  ...
  validatedItems.forEach((item) => {
    ...
    if (!ordersBySeller[sellerId]) {
      ordersBySeller[sellerId] = {
        sellerId,
        sellerInfo: item.productSnapshot.seller,
        items: [],
        totalAmount: 0,
      };
    }
    ...
    ordersBySeller[sellerId].totalAmount += item.priceAtTime * item.quantity;
  });
  ...
  for (const [sellerId, orderData] of Object.entries(ordersBySeller)) {
    // Generate a combined item name for display
    const itemNames = orderData.items.map((item) => item.productSnapshot.title);
    const displayName =
      itemNames.length > 1
        ? `${itemNames[0]} and ${itemNames.length - 1} other item(s)`
        : itemNames[0];
    ...
    const order = new Order({
      ...
      deliveryFee: 0, // Can be calculated based on seller's delivery settings
      ...
      pickupAddress: deliveryType === "pickup" ? pickupAddress : undefined,
      notes,
      productSnapshot: {
        cartItems: orderData.items.length,
    ...
    await savedOrder.populate([
      ...
      {
        path: "items.productId",
        select: "productTitle imageUrl productCategory",
      },
    ]);
    ...
    try {
      if (global.notificationService) {
        await global.notificationService.notifyNewOrder(savedOrder.sellerId, {
          _id: savedOrder._id,
  ...
  res.status(201).json(
    new ApiResponse(
      ...
      `${createdOrders.length} order(s) created successfully from cart`
    )
  );
});
